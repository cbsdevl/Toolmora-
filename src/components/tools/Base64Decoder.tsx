...
export default function Base64Decoder() {
  ...
  const getDecodedText = () => {
    ...
    try {
      // Decode Base64 safely supporting UTF-8 bytes
      const binString = atob(inputText.trim().replace(/\s/g, ""));
      ...
      if (!error) setError("Invalid Base64 string format. Please verify character symbols and padding (=).");
      return "";
    }
  };
  ...
  const handleDownload = () => {
    ...
    element.download = "base64-decoded.txt";
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm" id="base64-decoder-card">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Input Textarea */}
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700 block">Base64 Input String</label>
          <textarea
            ...
            placeholder="e.g. dG9vbG1vcmE= (paste Base64 string here)..."
          />
        </div>
        ...
        <div className="space-y-2 relative">
          <div className="flex justify-between items-center">
            <label className="text-sm font-semibold text-gray-700 block">Plain Text Output</label>
            {outputText && !error && (
          ...
          <textarea
            ...
            placeholder="Decoded text results will appear here..."
          />
          ...
          {error && (
            <div className="absolute left-3 right-3 bottom-3 bg-red-50 border border-red-100 p-3.5 rounded-xl flex gap-2.5 text-red-800 text-xs">
              ...
              <div className="space-y-0.5">
                <p className="font-bold">Decoding Failure:</p>
